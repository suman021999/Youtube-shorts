...
const Chatbox = ({ videoId, onClose }) => {
  ...
  // Update comment timestamps periodically
  useEffect(() => {
    const interval = setInterval(() => {
      setComments(prevComments =>
        prevComments.map(comment => ({
  ...
  // Load comments and setup socket connection
  useEffect(() => {
    const loadComments = async () => {
      try {
        setIsLoading(true);
    ...
    if (videoId) {
      ...
      const unsubscribe = chatService.subscribeToComments(videoId, (data) => {
        switch (data.type) {
          case 'NEW_COMMENT':
          ...
          case 'LIKE_COMMENT':
            setComments(prev => 
              prev.map(c => 
                c.id === data.commentId ? { ...c, likes: data.likes } : c
              )
            );
            break;
          case 'DISLIKE_COMMENT':
            setComments(prev => 
              prev.map(c => 
                c.id === data.commentId ? { ...c, dislikes: data.dislikes } : c
              )
            );
            break;
          case 'TYPING_START':
