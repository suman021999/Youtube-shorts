...
const Chatbox = ({ videoId }) => {
  ...
  // Connect socket and set up event listeners when videoId changes
  useEffect(() => {
    ...
    const loadInitialComments = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_CHAT_URL}/video/${videoId}`);
        if (response.ok) {
    ...
    socket.on('comment-liked', (likedComment) => {
      setComments(prev => 
        prev.map(comment => 
  ...
  const handleReply = (parentId, replyText) => {
    if (!replyText.trim() || !socket || !user) return;
  ...
  const handleSaveEdit = (id) => {
    if (!editText.trim() || !socket || !user) return;
