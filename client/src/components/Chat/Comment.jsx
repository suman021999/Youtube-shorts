...
//               <div 
//                 className={`absolute right-0 top-8 w-32 rounded-md shadow-lg py-1 z-10 ${isDarkMode ? 'dark:bg-[#303030]' : 'bg-white'}`}
//               >
//                 <button
//                   onClick={() => {
//                     handleEdit(comment);
...
//             {hasReplies && (
//               <button 
//                 className="text-gray-500 hover:bg-blue-200 p-2 mt-2 rounded-full text-xs flex items-center"
//                 onClick={() => setShowReplies(!showReplies)}
//               >
//                 <IoIosArrowDown className={`ml-1 w-4 h-4 transition-transform ${showReplies ? 'rotate-180' : ''}`} />
      ...
      <div className={`flex-1 p-2 rounded-lg relative ${isDarkMode && "dark:bg-[#121212e8]"}`}>
        <div className='flex justify-between'>
          <div className="flex items-center space-x-2">
            ...
            <span className="text-gray-500 text-xs">{calculateTimeAgo(comment.createdAt)}</span>
          </div>
          <div className="relative">
            <button 
        ...
        {editingCommentId === comment._id ? (
          ...
          <>
            ...
            <div className="flex items-center space-x-4 mt-1">
              <button 
                className="text-gray-500 flex items-center hover:text-gray-700 text-xs"
                onClick={() => handleLike(comment._id)}
              >
                <AiFillLike /> {comment.likes || 0}
              </button>
              <button 
                className="text-gray-500 flex items-center hover:text-gray-700 text-xs"
                onClick={() => handleDislike(comment._id)}
              >
                <AiFillDislike /> {comment.dislikes || 0}
              </button>
              <button 
                className="text-gray-500 hover:text-gray-700 text-xs"
                onClick={() => setShowReplyInput(!showReplyInput)}
