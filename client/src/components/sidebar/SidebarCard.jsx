...
const SidebarCard = () => {
  ...
  const renderAvatar = () => {
        ...
        return (
          <img
            ...
            onError={(e) => {
              ...
              // Create fallback if nextSibling doesn't exist
              const fallback = document.createElement('div');
              fallback.className = 'w-10 h-10 bg-gray-400 text-white flex items-center justify-center rounded-full';
              fallback.innerHTML = '<span class="text-sm">' + (user.name ? user.name.charAt(0).toUpperCase() : '?') + '</span>';
    ...
    // Fallback for users without avatar
    return (
      <div className="w-10 h-10 bg-gray-400 text-white flex items-center justify-center rounded-full cursor-pointer">
  ...
  return (
    <>
      ...
      <div className={`fixed bottom-0 left-0 right-0 lg:hidden flex justify-around items-center p-2 z-50 ${
        ...
        <div 
          className="w-10 h-10 rounded-full cursor-pointer overflow-hidden flex items-center justify-center"
          onClick={() => setShowLogoutPopup(!showLogoutPopup)}
        >
          {renderAvatar()}
        </div>
