...
//   try {
//     const response = await axios.get(`${import.meta.env.VITE_VIDEO_URL}/random/${id}`);
//     if (response.data && response.data._id) {
//       return response.data._id;
//     }
//     return 'random'; // fallback if no video is returned
//   } catch (error) {
//     console.error("Error fetching random video:", error);
//     return 'random';
//   }
...
//   const shouldHighlightShorts = location.pathname.includes("/shorts");
...
// <Link
//   to={`/shorts/random`} // Default to 'random' initially
//   className={`${
...
//     const randomId = await getRandomVideoId(); // Get random video ID
//     navigate(`/shorts/${randomId}`); // Navigate to the random video
//   }}
// >
//   <SiYoutubeshorts className={`h-6 w-6 mx-2 flex-shrink-0 ${isDarkMode ? "text-white" : ""}`} />
//   {isSidebarOpen && <h2 className={isDarkMode ? "text-white" : ""}>Shorts</h2>}
