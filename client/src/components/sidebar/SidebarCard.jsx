...
// navigate('/shorts', { state: { fromHome: true } });
...
const SidebarCard = () => {
  ...
  const getRandomVideoId = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_AUTH_URL}/video/all`);
      if (response.data && response.data.length > 0) {
        const randomIndex = Math.floor(Math.random() * response.data.length);
        return response.data[randomIndex]._id;
      }
      return 'default-id'; // fallback if no videos exist
    } catch (error) {
